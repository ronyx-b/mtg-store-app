...
export default function CartCardItem({item, card}) {
  ...
  return (<div className="CartItem">
    <Row className="m-1 p-2 border-bottom">
      {card && <>
        <Col xs={5} md="auto">
          <Link href={`/CardDetails/${card.id}`} style={{ color: "#000000", textDecoration: "none"}}>
        ...
        <Col xs={7} md>
          <Row><Link href={`/CardDetails/${card.id}`} style={{ color: "#000000", textDecoration: "none", fontWeight: "bold"}}>{card.name}</Link></Row>
